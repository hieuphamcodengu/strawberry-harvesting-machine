...
// Biến lưu giá trị yaw
float currentYaw = 0;
float targetYaw = 0;
bool keepStraight = false;  // Đang giữ hướng đi thẳng
...
void calibrateGyro() {
  ...
  Serial.println(F("=== CALIB GYRO ==="));
  Serial.println(F("Giu IMU dung yen..."));
  delay(1000);
  ...
  Serial.println(F("Calib gyro xong!"));
}
...
void setup_MPU() {
  Wire.begin();
  Serial.println(F("=== KHOI TAO MPU9250 ==="));
...
void updateMPU() {
  ...
  float dt = (now - lastMPUUpdate) * 1e-6f;
  lastMPUUpdate = now;
  ...
  currentYaw = atan2(2 * (q1 * q2 + q0 * q3),
...
/* ================= Get Yaw Correction ================= */
// Trả về giá trị hiệu chỉnh cho RPM (dương = tăng bên trái, giảm bên phải)
float getYawCorrection() {
  if (!keepStraight) return 0;

  // Tính sai số góc
  float errorYaw = targetYaw - currentYaw;

  // Chuẩn hóa về [-180, 180]
  while (errorYaw > 180) errorYaw -= 360;
  while (errorYaw < -180) errorYaw += 360;

  // P controller đơn giản
  const float Kp_yaw = 1.0f;  // Điều chỉnh thực nghiệm (0.5 - 2.0)
  float correction = Kp_yaw * errorYaw;

  // Giới hạn hiệu chỉnh ±20 RPM
